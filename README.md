...
Reviewer grading uses the panel's synthesis as an approximate answer key rather than independent ground truth, so a grade partly measures agreement with the other reviewers, not correctness --- good enough to catch a shallow or non-responsive review, not reliable for telling a wrong finding from a correct one that only one reviewer caught. Roster decisions stay human-made, with grades as one input among several, never an automated gate. Mechanism, failure modes, and ranked improvement options (a git-history backtest is the recommended one) are tracked in [`KNOWN_ISSUES.md`](KNOWN_ISSUES.md). This grading is an operational shortcut for running the hub day to day, separate from the [review-diversity research](https://writingcommitmentdevice.substack.com/p/are-two-models-better-than-one-for) (2026-06-16), which pre-registers real, manually verified bug-fix pairs as its ground truth.
